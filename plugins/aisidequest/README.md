...
- 저장: 이벤트명, UUID 이벤트 ID, 기기별 증가 sequence, 해시된 세션/턴 식별자, 관측 시각, 마지막 폴더명과 고정 목록의 명령 분류
- 저장하지 않음: 프롬프트, 응답, 소스 코드, 전체 파일 경로, 원본 명령·인자, transcript, 도구 입력/출력
- `cwd`는 로컬에서 마지막 폴더명만 최대 64자로 정제하고, 명령은 `npm test`, `git status`, `Gradle test`, `코드 변경`, `기타 명령` 같은 고정 label로 바꾼 뒤 원문을 즉시 폐기합니다.
