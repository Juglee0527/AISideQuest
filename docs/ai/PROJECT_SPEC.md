...
- 현재 상태: 실사용 베타 Task 1~19 구현 완료, Task 20 외부 증거 대기, Discover Task 21~24 완료
- 애플리케이션 버전: `0.1.0`
- 최종 현행화: 2026-07-20
...
## 4.5 Discover API와 Hacker News source
...
Hacker News adapter는 exact HTTPS host에서 feed 4개와 feed별 최대 12개 item을 가져온다. 삭제·dead·누락 item은 개별 제외하고 중복 ID는 한 번만 요청한다. Fresh TTL은 10분, maximum stale은 24시간이다. Remotive와 나머지 source는 아직 외부 network를 호출하지 않는다.
...
- Remotive와 추가 Discover source, screen, saved item, interest, analytics 구현
...
2026-07-21 기준 자동 test 182개가 통과했다.
...
Task 21에서 제품 규칙을 확정했고 Task 22에서 common model과 인증된 read API를, Task 23에서 safe adapter·HTTP·normalization·cache·stale 기반을 구현했다. Task 24에서 Hacker News Top·Ask·Show·Jobs item을 활성화했으며 화면은 아직 제공하지 않는다.
...
다음 작업은 Task 25의 Remotive 수익 기회 adapter 구현이다.
...
- Task 25~26: Remotive와 screen 미구현
