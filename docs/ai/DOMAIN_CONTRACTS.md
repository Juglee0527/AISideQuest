...
- Saved lists remain available during source/cache failure. Saved snapshots and explicit interests are exported in user-data schema version 3 and removed by account deletion.
- Interest updates replace at most 10 tags from a fixed 20-tag allowlist and require CSRF plus UUID idempotency. Empty selection restores chronological ordering and removes the preference row.
- Personalized order is deterministic: explicit tag-match count, relative recency band, source engagement, clear reward or salary, then chronological tie-breakers. Recommendation reasons expose only those factors, and a cursor is invalid after interests change.
- Personalization never consumes prompts, AI responses, code, diffs, transcripts, commands, tool input/output, workspace labels, or local paths.
