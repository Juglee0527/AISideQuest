...
- Fresh·maximum stale 기준은 Hacker News 10분·24시간, Remotive 6시간·72시간, DEV 30분·24시간, Stack Overflow 15분·24시간, GitHub 30분·24시간이다. 정규화 cache는 마지막 성공 갱신 후 최대 7일 안에 교체하거나 삭제한다.
- `GITHUB_DISCOVER_TOKEN`은 OAuth client secret이나 사용자 access token을 재사용하지 않는 별도 server-only secret이다. 브라우저·API 응답·로그·metric label에 노출하지 않으며 organization/repository allowlist가 없으면 adapter를 등록하지 않는다.
