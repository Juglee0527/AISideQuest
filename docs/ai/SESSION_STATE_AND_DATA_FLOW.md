...
플러그인 hook 명령은 시작된 작업이 받은 버전별 `PLUGIN_ROOT`를 그대로 고정해서 실행하지 않는다. 해당 설치의 plugin cache 상위 디렉터리에서 `record-event.mjs`가 존재하는 가장 최근 runtime을 선택하고, 찾지 못한 경우에만 원래 `PLUGIN_ROOT`로 돌아간다. 따라서 로컬 개발 중 cachebuster 재설치가 이전 버전 cache를 제거해도 이미 열려 있던 다른 저장소의 Codex 작업은 다음 hook부터 현재 설치 runtime으로 계속 전달한다. 새 hook 정의·스킬·도구 자체를 읽는 경계는 Codex 계약대로 앱 재시작 후 새 작업이다.
...
- 플러그인 업데이트 뒤 특정 기존 작업만 감지되지 않으면 queue나 API보다 먼저 그 작업이 보유한 hook 정의와 `PLUGIN_ROOT` cache 생존 여부를 확인한다. 최신 hook은 현재 설치 runtime을 다시 찾지만, 이 복구 로직이 포함되기 전에 시작된 작업은 앱 재시작 후 새 작업으로 전환해야 한다.
