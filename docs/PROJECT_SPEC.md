...
통계 기간은 브라우저 로컬 시간을 기준으로 하며 주간은 월요일부터 시작한다.
AI 대기 시간은 진행 중 세션을 포함하고, 기간 경계를 넘는 세션은 해당 기간과 실제로 겹친 시간만 합산한다.
퀘스트 수와 예상 포인트는 `completedAt`이 선택한 기간에 포함되는 완료 이력으로 계산한다.
예상 절약 시간은 계산 기준이 확정되기 전까지 `-`로 표시한다.
