...
현재는 브라우저 MVP, NestJS API, PostgreSQL, GitHub OAuth, AI 세션 API와 AISideQuest Codex 플러그인의 자동 감지·장애 복구를 완료한 실사용 베타 개발 단계입니다. lifecycle event는 durable FIFO queue와 30초 heartbeat로 전송되며 서버가 비정상 종료, 만료와 late `Stop`을 복구합니다. 자동 감지를 사용할 수 없을 때는 수동 모드를 유지합니다. 다음 작업은 DB 기반 퀘스트 목록 API입니다.
